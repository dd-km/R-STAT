...
server <- function(input, output) {
  ...
  # Filter Data
  fd <- filter_data_server(
    id = "filter",
    data = reactive(id$data()),
    vars = reactive(names(id$data())),
    widget_char = "virtualSelect"
  )
  
  # Edit data
  ed <- edit_data_server(
    id = "ed",
    data_r = reactive(fd$filtered()),
    add = TRUE,
    update = TRUE,
    delete = TRUE,
    download_csv = TRUE,
  ...
  # Update Data
  data <- update_variables_server(
    id = "vars",
    data = reactive(data.frame(ed()[,-1], row.names=ed()[,1]))
  )
...
# Selekcija podataka
  ...
  output$zz <- renderDataTable({
    ...
    dtt(Z) |> formatStyle('MEAN_Z', fontWeight = 'bold')
  })
  ...
  output$tt <- renderDataTable({
    ...
    dtt(tt) |> formatStyle('MEAN_T', fontWeight = 'bold')
  })
  ...
  output$ll <- renderDataTable({
    ...
    dtt(L) |> formatStyle('MEAN_L', fontWeight = 'bold')
  })
  ...
  output$tpro <- renderPlotly({
    ...
    colnames(Te) <- c("Variables", "T")
    tpro <- ggplot(data=Te, aes(x=Variables, y=T, fill=Variables, color=Variables)) +
      geom_bar(stat="identity", alpha = input$opa, width = input$width) +
  ...
  output$lpro <- renderPlotly({
    ...
    Lns <- Zns*0.83+3
    Los <- Zos*0.83+3
    L <- cbind.data.frame(round(Lns, digits = 2), round(Los, digits = 2))
    vars <- names(L)
    Lt <- t((L[input$ENTITETI,]))
    Le <- cbind.data.frame(vars, Lt)
    colnames(Le) <- c("Variables", "L")
    lpro <- ggplot(data=Le, aes(x=Variables, y=L, fill=Variables, color=Variables)) +
      geom_bar(stat="identity", alpha = input$opa, width = input$width) +
  ...
  output$tf <- renderDataTable({
    ...
    dtf(tf) |> formatRound(c('Observed Percent', 'Expected Frequency',"Expected Percent"), 2)
  })
  ...
  output$pie <- renderPlotly({
    ...
    pie <- plot_ly(labels = x, 
                   ...
                   showlegend = T,
                   marker = list(colors = colors, line = list(color = colors, width = 0.5)
                   )
            )
    gtheme(pie)
  })
  ...
  output$graf3 <- renderPlotly({
    ...
    if(input$ori2=="x") {
      graf3 <- ggplot(my_data, aes(x=var1, fill = var2, color = var2)) + 
        ...
        labs(x= input$CT1, y = "p", fill = input$CT2, color = input$CT2) +
        theme(panel.background = element_rect(fill = "#ffffff")) +
        theme_minimal() +
        scale_fill_manual(values = colors) +
        scale_color_manual(values = colors)
      ...
      graf3 <- ggplot(my_data, aes(y=var1, fill = var2, color = var2)) + 
        geom_bar (alpha = input$opa, position="fill", width = input$width)+
        labs(y= input$CT1, x = "p", fill = input$CT2, color = input$CT2) +
        theme(panel.background = element_rect(fill = "#ffffff")) +
        theme_minimal() +
        scale_fill_manual(values = colors) +
        scale_color_manual(values = colors)
  ...
  output$graf5 <- renderPlotly({
    ...
    if(input$ori3=="x") {
      graf5 <-  ggplot(my_data, aes(x=var1, fill = var2, color = var2)) + 
        geom_bar (alpha = input$opa, position = "fill", width = input$width)+
        labs(x = input$MCN1, y = "p", fill = input$MCN2, color = input$MCN2) +
        theme(panel.background = element_rect(fill = "#ffffff")) +
        theme_minimal() +
        scale_fill_manual(values = colors) +
        scale_color_manual(values = colors)
      ...
      graf5 <- ggplot(my_data, aes(y=var1, fill = var2, color = var2)) + 
        geom_bar (alpha = input$opa, position = "fill", width = input$width)+
        labs(y = input$MCN1, y = "p", fill = input$MCN2, color = input$MCN2) +
        theme(panel.background = element_rect(fill = "#ffffff")) +
        theme_minimal() +
        scale_fill_manual(values = colors) +
        scale_color_manual(values = colors)
  ...
  output$r <- renderDataTable({
    ...
    upper.tri(R)
    R[upper.tri(R)] <- NA
    dtt(R) |> formatRound(colnames(R), digits=3) |> 
  ...
  output$p <- renderDataTable({
    ...
    upper.tri(P)
    P[upper.tri(P)] <- NA
    dtt(P) |> formatRound(colnames(P), digits=4) |>
      formatStyle(var_name_numeric, 
                  color = styleInterval(
                    c(0, 0.05), 
  ...
  output$scatter <- renderPlotly({
    ...
    X <- my_data[, input$XX]
    Y <- my_data[, input$YY]
    scatter <- ggplot(my_data, aes(x=X, y=Y)) +
  ...
  output$dest <- renderDataTable({
    ...
    dtf(dest) |> formatRound(c("MEAN", "SD", "SEM", "-CI95%", "+CI95%", "S-W", "P"), 3)
  })
  ...
  output$dest2 <- renderDataTable({
    ...
    dtf(dest2) |> formatRound(c("MEAN", "SD", "SEM", "-CI95%", "+CI95%"), 3)
  })
  ...
  output$desa <- renderDataTable({
    ...
    dtf(desa) |> formatRound(c("MEAN", "SD", "SEM", "-CI95%", "+CI95%", "S-W", "P"), 3)
  })
  ...
  output$anovat <- renderDataTable({
    ...
    dtt(aov[[1]]) |> formatRound(c("Sum of Squares", "Mean Square", "F", "p"), 3)
  ...
  output$PH <- renderDataTable({
   ...
   dtt(ph) |> formatRound(c("Mean difference", "SE", "t", "p"), 3)
  })
  ...
  output$desrma <- renderDataTable({
    ...
    dtf(desrm) |> formatRound(c("MEAN", "SD", "SEM", "-CI95%", "+CI95%", "S-W", "P"), 3)
  })
  ...
  output$aovrm <- renderDataTable({
    ...
    dtt(aovrm) |> formatRound(c("Sum of Squares", "Mean Square", "F", "p"), 3)
  }) 
  ...
  output$pwc <- renderDataTable({
    ...
    dtt(pwc) |> formatRound(c("Mean difference", "SE", "t", "p"), 3)
  })
  ...
  output$rs <- renderDataTable({
    ...
    dtt(rs) |> formatRound(colnames(rs), digits=3) 
  })
  ...
  output$rez <- renderDataTable({
    ...
    dtt(rez) |> formatRound(colnames(rez), digits=3) 
  })
  ...
  output$dep <- renderDataTable({
    ...
    dtt(dep) |> formatRound(colnames(dep), digits=3) 
  })
  ...
  output$Lamda <- renderDataTable({
    ...
    SSMC<- SMC %*% j
    ssmc <- SSMC[1,1]
    dtt(Lamda) |> formatRound(colnames(Lamda), digits=3) |>
  ...
  output$A <- renderDataTable({
    ...
    dtt(A) |> formatRound(colnames(A), digits=3) 
  ...
  output$FF <- renderDataTable({
    ...
    dtt(FF) |> formatRound(colnames(FF), digits=3) 
  ...
  output$M <- renderDataTable({
    ...
    dtt(M) |> formatRound(colnames(M), digits=3)
  })
  ...
  output$Scores <- renderDataTable({
    ...
    dtt(FS) |> formatRound(colnames(FS), digits=3)
  })
  ...
  output$irs <- renderDataTable({
    ...
    dtt(irs) |> formatRound(colnames(irs), digits=3)
  })
  output$cd <- renderDataTable({
    ...
    dtt(cd) |> formatRound(colnames(cd), digits=3)
  })
